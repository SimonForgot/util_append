...
void process_bip_ctrl() {
    Interface17 *ip = GetCOREInterface17(); 
    ip->PushPrompt(_T("Processing Biped Control..."));
}

PYBIND11_MODULE(BipedCOMValueScalePy, m) {
    m.doc() = "Pybind11 example for processing Biped Control";
    m.def("process_bip_ctrl", &process_bip_ctrl, "Process a Biped Control object");
}